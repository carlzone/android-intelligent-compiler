...
- Add multiple screens, explicit navigation/back/finish operations, lifecycle-safe restoration, resources, themes, localization, and accessibility semantics. IR 0.2 now supports exhaustive portrait/landscape and compact/expanded `on_create` variants at a fixed 600dp breakpoint, typed Bundle restoration for declared scalar state, stable generated view IDs for hierarchy restoration, and password-state exclusion. It requires informative images and progress controls to have content descriptions, requires decorative images to be explicit, and restricts enabled state to interactive controls.
